 ...
 * Plugin URI: https://github.com/gl0bal01/wordpress-plugins/github-sync
 * Description: Automatically sync WordPress posts to GitHub as markdown files using cronjobs.
 * Version: 1.0.0
 * Author: gl0bal01
...
define('GITHUB_SYNC_VERSION', '1.0.0');
define('GITHUB_SYNC_PLUGIN_FILE', __FILE__);
define('GITHUB_SYNC_PLUGIN_DIR', plugin_dir_path(__FILE__));
define('GITHUB_SYNC_PLUGIN_URL', plugin_dir_url(__FILE__));
define('GITHUB_SYNC_PLUGIN_BASENAME', plugin_basename(__FILE__));
...
class GitHubSyncPlugin
{
     ...
     */
    private const OPTION_PREFIX = 'github_sync_';

    /**
     * Cron hook name
     */
    private const CRON_HOOK = 'github_sync_cron';

    /**
     * Webhook endpoint namespace
     */
    private const WEBHOOK_NAMESPACE = 'github-sync/v1';
    ...
    private function init(): void
    {
        ...
        register_activation_hook(GITHUB_SYNC_PLUGIN_FILE, [$this, 'activate']);
        register_deactivation_hook(GITHUB_SYNC_PLUGIN_FILE, [$this, 'deactivate']);
    ...
    public function loadTextDomain(): void
    {
        load_plugin_textdomain(
            'github-sync',
            false,
            dirname(GITHUB_SYNC_PLUGIN_BASENAME) . '/languages'
        );
    }
    ...
    public function addCustomCronInterval(array $schedules): array
    {
        $schedules['every_15_minutes'] = [
            ...
            'display'  => __('Every 15 minutes', 'github-sync'),
        ];
    ...
    private function createSyncLogTable(): void
    {
        ...
        $tableName = $wpdb->prefix . 'github_sync_log';
    ...
    {
        add_options_page(
            __('GitHub Sync Settings', 'github-sync'),
    ...
    {
        // Register settings
        register_setting('github_sync_settings', self::OPTION_PREFIX . 'github_token', [
            'type' => 'string',
            'sanitize_callback' => 'sanitize_text_field',
        ]);
        
        register_setting('github_sync_settings', self::OPTION_PREFIX . 'github_repo', [
            'type' => 'string',
            'sanitize_callback' => 'sanitize_text_field',
        ]);
        
        register_setting('github_sync_settings', self::OPTION_PREFIX . 'sync_interval', [
            'type' => 'string',
            'sanitize_callback' => 'sanitize_text_field',
        ]);
        
        register_setting('github_sync_settings', self::OPTION_PREFIX . 'sync_enabled', [
            'type' => 'boolean',
            'sanitize_callback' => 'rest_sanitize_boolean',
        ]);
        
        register_setting('github_sync_settings', self::OPTION_PREFIX . 'sync_published_only', [
            'type' => 'boolean',
            'sanitize_callback' => 'rest_sanitize_boolean',
        ]);
    }
    ...
    {
        register_setting('github_sync_settings', self::OPTION_PREFIX . 'bidirectional_sync', [
            'type' => 'boolean',
            'sanitize_callback' => 'rest_sanitize_boolean',
        ]);
        
        register_setting('github_sync_settings', self::OPTION_PREFIX . 'webhook_secret', [
            'type' => 'string',
            'sanitize_callback' => 'sanitize_text_field',
        ]);
    }
    ...
    {
        if (isset($_POST['manual_sync']) && wp_verify_nonce($_POST['_wpnonce'], 'github_sync_manual')) {
            $this->triggerManualSync();
            echo '<div class="notice notice-success"><p>' . 
                 esc_html__('Manual sync triggered successfully!', 'github-sync') . '</p></div>';
        }
        ...
        ?>
        <div class="wrap">
            <h1><?php echo esc_html__('GitHub Sync Settings', 'github-sync'); ?></h1>
            
            <form method="post" action="options.php">
                <?php settings_fields('github_sync_settings'); ?>
                <table class="form-table">
                    <tr>
                        <th scope="row">
                            <label for="github_token"><?php esc_html_e('GitHub Personal Access Token', 'github-sync'); ?></label>
                        </th>
                        <td>
                            <input type="password" 
                                   id="github_token" 
                                   ...
                                   class="regular-text" />
                            <p class="description">
                                <?php esc_html_e('Generate a token at GitHub Settings > Developer settings > Personal access tokens', 'github-sync'); ?>
                            </p>
                        </td>
                    </tr>
                    ...
                    <tr>
                        <th scope="row">
                            <label for="github_repo"><?php esc_html_e('GitHub Repository', 'github-sync'); ?></label>
                        </th>
                        <td>
                            <input type="text" 
                                   id="github_repo" 
                            ...
                            <p class="description">
                                <?php esc_html_e('Format: username/repository-name', 'github-sync'); ?>
                            </p>
                        </td>
                    </tr>
                    ...
                    <tr>
                        <th scope="row">
                            <label for="sync_interval"><?php esc_html_e('Sync Interval', 'github-sync'); ?></label>
                        </th>
                        <td>
                    ...
                    <tr>
                        <th scope="row">
                            <?php esc_html_e('Sync Options', 'github-sync'); ?>
                        </th>
                        <td>
                            <fieldset>
                                <label for="sync_enabled">
                                    ...
                                    <?php esc_html_e('Enable automatic syncing', 'github-sync'); ?>
                                </label><br />
                                ...
                                <label for="sync_published_only">
                                    ...
                                    <?php esc_html_e('Sync published posts only', 'github-sync'); ?>
                                </label><br />
                                ...
                                <label for="bidirectional_sync">
                                    ...
                                    <?php esc_html_e('Enable bidirectional sync (GitHub → WordPress)', 'github-sync'); ?>
                                </label>
                            </fieldset>
                        </td>
                    </tr>
                    ...
                    <tr>
                        <th scope="row">
                            <label for="webhook_secret"><?php esc_html_e('Webhook Secret', 'github-sync'); ?></label>
                        </th>
                        <td>
                            <input type="password" 
                                   id="webhook_secret" 
                            ...
                            <p class="description">
                                <?php esc_html_e('Secret key for webhook security. Generate a random string.', 'github-sync'); ?>
                            </p>
                            <button type="button" onclick="document.getElementById('webhook_secret').value = Math.random().toString(36).substring(2, 15) + Math.random().toString(36).substring(2, 15);" class="button button-secondary">
                                <?php esc_html_e('Generate Secret', 'github-sync'); ?>
                            </button>
                        </td>
                    </tr>
                    ...
                    <tr>
                        <th scope="row">
                            <?php esc_html_e('Webhook URL', 'github-sync'); ?>
                        </th>
                        <td>
                            <code><?php echo esc_html($webhookUrl); ?></code>
                            <p class="description">
                                <?php esc_html_e('Use this URL when setting up the webhook in your GitHub repository.', 'github-sync'); ?>
                            </p>
                            <button type="button" onclick="navigator.clipboard.writeText('<?php echo esc_js($webhookUrl); ?>');" class="button button-secondary">
                                <?php esc_html_e('Copy URL', 'github-sync'); ?>
                            </button>
                        </td>
                    </tr>
                    <?php endif; ?>
            ...
            <h2><?php esc_html_e('Manual Sync', 'github-sync'); ?></h2>
            <form method="post">
                <?php wp_nonce_field('github_sync_manual'); ?>
                <p>
                    <?php esc_html_e('Trigger a manual sync of all eligible posts.', 'github-sync'); ?>
                </p>
                <input type="submit" 
                       name="manual_sync" 
                       class="button button-secondary" 
                       value="<?php esc_attr_e('Sync Now', 'github-sync'); ?>" />
            </form>
            
            <hr />
            
            <h2><?php esc_html_e('Sync Status', 'github-sync'); ?></h2>
            <?php $this->renderSyncStatus(); ?>
            
            <?php if ($bidirectionalSync): ?>
            <hr />
            <h2><?php esc_html_e('GitHub Webhook Setup', 'github-sync'); ?></h2>
    ...
    private function renderSyncStatus(): void
    {
        ...
        if (empty($logs)) {
            echo '<p>' . esc_html__('No sync logs found.', 'github-sync') . '</p>';
            return;
        }
        ?>
        <table class="wp-list-table widefat fixed striped">
            <thead>
                <tr>
                    <th><?php esc_html_e('Post ID', 'github-sync'); ?></th>
                    <th><?php esc_html_e('Post Title', 'github-sync'); ?></th>
                    <th><?php esc_html_e('Status', 'github-sync'); ?></th>
                    <th><?php esc_html_e('GitHub Path', 'github-sync'); ?></th>
                    <th><?php esc_html_e('Last Sync', 'github-sync'); ?></th>
                    <th><?php esc_html_e('Error', 'github-sync'); ?></th>
                </tr>
            </thead>
            <tbody>
                <?php foreach ($logs as $log): ?>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <?php else: ?>
                                <em><?php esc_html_e('Post not found', 'github-sync'); ?></em>
                            <?php endif; ?>
                        </td>
                        <td>
                        ...
                        <td>
                            ...
                            <?php else: ?>
                                <em><?php esc_html_e('Not synced', 'github-sync'); ?></em>
                            <?php endif; ?>
                        </td>
                        <td>
                            <?php 
                            if ($log['last_sync_time']) {
                                ...
                                echo '<em>' . esc_html__('Never', 'github-sync') . '</em>';
                            }
                            ?>
                        </td>
                        <td>
                            <?php if ($log['error_message']): ?>
                                <details>
                                    <summary><?php esc_html_e('View Error', 'github-sync'); ?></summary>
    ...
    public function processSyncQueue(): void
    {
        ...
        if (!$this->isConfigured()) {
            error_log('GitHub Sync: Plugin not properly configured');
            return;
        }
    ...
    private function syncPost(int $postId): void
    {
        ...
        try {
            ...
            error_log('GitHub Sync Error for post ' . $postId . ': ' . $e->getMessage());
        }
    }
    ...
    private function renderWebhookInstructions(string $webhookUrl, string $webhookSecret): void
    {
        $repo = $this->getOption('github_repo');
        if (empty($repo)) {
            echo '<p><em>' . esc_html__('Configure your GitHub repository first.', 'github-sync') . '</em></p>';
            return;
        }
        
        if (empty($webhookSecret)) {
            echo '<div class="notice notice-warning"><p>' . 
                 esc_html__('Please generate and save a webhook secret first.', 'github-sync') . '</p></div>';
            return;
        }
        ...
        <div class="webhook-instructions">
            <p><?php esc_html_e('To enable bidirectional sync, set up a webhook in your GitHub repository:', 'github-sync'); ?></p>
            
            <ol>
                <li>
                    <strong><?php esc_html_e('Go to your GitHub repository:', 'github-sync'); ?></strong>
                ...
                <li>
                    <strong><?php esc_html_e('Click "Add webhook"', 'github-sync'); ?></strong>
                </li>
                
                <li>
                    <strong><?php esc_html_e('Configure webhook settings:', 'github-sync'); ?></strong>
                    <ul style="margin-top: 10px;">
                        <li><strong>Payload URL:</strong> <code><?php echo esc_html($webhookUrl); ?></code></li>
                        <li><strong>Content type:</strong> application/json</li>
                        <li><strong>Secret:</strong> <em><?php esc_html_e('Use the webhook secret from above', 'github-sync'); ?></em></li>
                        <li><strong>Events:</strong> <?php esc_html_e('Just the push event', 'github-sync'); ?></li>
                        <li><strong>Active:</strong> ✅ <?php esc_html_e('Checked', 'github-sync'); ?></li>
                    </ul>
                </li>
            ...
            <div class="notice notice-info">
                <p>
                    <strong><?php esc_html_e('Important:', 'github-sync'); ?></strong>
                    <?php esc_html_e('Only markdown files in the /posts/ directory will trigger WordPress updates. Changes to other files will be ignored.', 'github-sync'); ?>
                </p>
            </div>
        </div>
    ...
    public function handleWebhook(\WP_REST_Request $request): \WP_REST_Response
    {
        ...
        if (!$this->verifyWebhookSignature($payload, $signature)) {
            error_log('GitHub Sync: Invalid webhook signature');
            return new \WP_REST_Response([
    ...
    private function updatePostFromGitHub(string $filePath): void
    {
        try {
            ...
            if (!$content) {
                error_log("GitHub Sync: Could not fetch content for {$filePath}");
                return;
            }
            ...
            if (!$parsedContent) {
                error_log("GitHub Sync: Could not parse markdown for {$filePath}");
                return;
            }
            ...
            if (!$postId) {
                error_log("GitHub Sync: No corresponding WordPress post found for {$filePath}");
                return;
            }
            ...
            if ($updateResult && !is_wp_error($updateResult)) {
                ...
                error_log("GitHub Sync: Successfully updated post {$postId} from GitHub");
                
                // Update sync log
                $this->updateSyncLog($postId, 'updated_from_github', $filePath, null);
            } else {
                error_log("GitHub Sync: Failed to update post {$postId}");
            }

        } catch (\Exception $e) {
            error_log("GitHub Sync: Error updating post from GitHub: " . $e->getMessage());
        }
    }
